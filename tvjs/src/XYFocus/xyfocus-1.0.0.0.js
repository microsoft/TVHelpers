...
;(function() {
    ...
    var CrossDomainMessageConstants = {
        messageDataProperty: "xyfocusControlMessage",
        register: "register",
    ...
    **/
    function _xyfocus(direction, keyCode, referenceRect, dontExit) {
        ...
        if (result && _trySetFocus(result.target, keyCode)) {
            ...
            if (result.target.nodeName === "IFRAME") {
                ...
                // If the target element has a navigateFocus function then use that
                // to communicate focus transition into the element. Currently only
                // webview elements have this function.
                var refRect = _toIRect({
                    left: result.referenceRect.left - result.targetRect.left,
                    top: result.referenceRect.top - result.targetRect.top,
                    width: result.referenceRect.width,
                ...
                result.target.navigateFocus(direction, WebViewHelper.refRectToNavigateFocusRect(refRect));
            }
            ...
            if ((!dontExit && top !== window) || typeof global.departFocus === "function") {
                ...
                // If we're the top level frame but we're in a webview that supports departFocus, then we call departFocus
                // to let our host app know that we want to give up focus.
                if (top === window && typeof global.departFocus === "function") {
                    global.departFocus(direction, WebViewHelper.refRectToNavigateFocusRect(refRect));
                }
                else {
                    var message = {};
                    message[CrossDomainMessageConstants.messageDataProperty] = {
    ...
    function _findNextFocusElementInternal(direction, options) {
        ...
        // Handle override.
        if (refObj.element) {
            var manualOverrideOptions = refObj.element.dataset.xyfocus;
    ...
    (function(KeyHandlerStates) {
        ...
        ToggleModeRestState.accept = function(element) {
            element.classList.add(ClassNames.toggleModeActive);
            return true;
        };
    ...
    (function(WebViewHelper) {
        function refRectToNavigateFocusRect(refRect) {
            // Temporary workaround to create a ClientRect for the navigateFocus APIs.
            var element = document.createElement("div");
            element.style.position = "fixed";
            element.style.left = refRect.left + "px";
            element.style.top = refRect.top + "px";
            element.style.width = refRect.width + "px";
            element.style.height = refRect.height + "px";
        ...
        function navigateFocusRectToRefRect(navigateFocusRect) {
            return _toIRect({
                left: navigateFocusRect.left,
                top: navigateFocusRect.top,
                width: navigateFocusRect.right - navigateFocusRect.left,
                height: navigateFocusRect.bottom - navigateFocusRect.top
            });
        }
    ...
    if (global.document) {
        ...
        document.addEventListener("departingfocus", function(eventArg) {
            var focusChanged = _xyfocus(
                ...
                -1,
                WebViewHelper.navigateFocusRectToRefRect(eventArg.origin)
            );
        ...
        // into the webview from the app.
        document.addEventListener("navigatingfocus", function(eventArg) {
            var focusChanged = _xyfocus(
                eventArg.navigationReason,
                -1,
                WebViewHelper.navigateFocusRectToRefRect(eventArg.origin)
        ...
        assign(xyfocus, eventMixin);
        xyfocus._listeners = {};
        xyfocus.Key = Keys;

        var eventSrc = xyfocus;
        global.xyfocus = xyfocus;
    }

    if (global.navigator && typeof global.navigator.gamepadInputEmulation === "string") {
        global.navigator.gamepadInputEmulation = "keyboard";
    }
}.call(this));
